...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes float2     { 0%,100%{transform:translate(0,0)} 33%{transform:translate(-20px,15px)} 66%{transform:translate(10px,-10px)} }
  @keyframes pulse      { 0%,100%{opacity:1;box-shadow:0 0 0 3px rgba(76,175,80,0.2)} 50%{opacity:0.6;box-shadow:0 0 0 6px rgba(76,175,80,0.05)} }
  ...
  .cursor-ring {
    ...
    transition: all 0.15s ease-out; opacity: 0.6;
  }
  .grain-overlay {
  ...
  .tag-chip {
    background: var(--surface2); color: var(--ink2); font-size: 0.68rem;
    padding: 0.25rem 0.7rem; border-radius: 2px; letter-spacing: 0.05em;
    font-family: var(--mono); border: 1px solid var(--border);
  }

  .section-label {
    font-size: 0.68rem; letter-spacing: 0.18em; text-transform: uppercase;
    color: var(--ink3); display: flex; align-items: center; gap: 12px;
  ...
  .nav-link {
    font-size: 0.73rem; font-weight: 300; letter-spacing: 0.12em; text-transform: uppercase;
    text-decoration: none; color: var(--ink3); transition: color 0.2s;
  ...
  .project-card:hover .card-img-zone img {
  ...
  .card-arrow {
    ...
    color: var(--accent); background: rgba(14,13,11,0.55);
    backdrop-filter: blur(6px); z-index: 2;
    opacity: 0; transform: scale(0.75) rotate(-45deg);
  ...
  .skill-pill {
    ...
    padding: 0.45rem 1.1rem; border-radius: 99px; font-size: 0.8rem;
    font-weight: 300; transition: all 0.2s; cursor: default;
    border: 1px solid var(--border2); background: transparent;
    color: var(--ink2); font-family: var(--sans); letter-spacing: 0.01em;
  }
  .skill-pill.featured { background: var(--ink); color: var(--surface); border-color: var(--ink); }
  .skill-pill:hover { border-color: var(--accent); color: var(--accent2); }
  .skill-pill.featured:hover { background: var(--accent2); border-color: var(--accent2); color: var(--surface); }

  .input-field {
    background: transparent; border: 1px solid var(--border2); border-radius: 3px;
    padding: 0.85rem 1.1rem; font-family: var(--sans); font-size: 0.88rem;
    color: var(--ink); outline: none; width: 100%; font-weight: 300; transition: border-color 0.2s;
  }
  .input-field:focus { border-color: var(--accent); }
  .input-field::placeholder { color: var(--ink4); }

  .btn-primary {
    background: var(--ink); color: var(--surface); border: none; border-radius: 2px;
    padding: 0.9rem 2.2rem; font-size: 0.76rem; font-weight: 500; letter-spacing: 0.1em;
  ...
  .btn-outline {
    ...
    border-radius: 2px; padding: 0.9rem 2.2rem; font-size: 0.76rem; font-weight: 400;
    letter-spacing: 0.1em; text-transform: uppercase; cursor: pointer; font-family: var(--sans);
    transition: all 0.2s; display: inline-flex; align-items: center; gap: 10px; text-decoration: none;
  }
  .btn-outline:hover { border-color: var(--ink); color: var(--ink); background: var(--surface2); }

  .stat-number { font-family: var(--serif); font-size: 2rem; font-weight: 400; line-height: 1; color: var(--ink); display: block; }
  .stat-label  { font-family: var(--mono); font-size: 0.67rem; color: var(--ink3); letter-spacing: 0.1em; text-transform: uppercase; margin-top: 6px; display: block; }
...
const projects = [
  {
    num: "01", name: "Library Management Desktop App", year: "2023", role: "Backend Developer",
    ...
    live: "", github: "https://github.com/newbieflank/perpusmu.git",
    ...
    highlights: ["Developed CRUD system for members, staff, and book management", "Integrated RFID-based authentication for secure user access", "Optimized database structure using MySQL for efficient data handling", "Implemented role-based access control for different user types", "Ensured data consistency and validation across all modules"],
    challenge: "Integrating RFID authentication with the desktop application and ensuring reliable data communication between hardware and system, while maintaining data integrity in the database."
  },
  {
    num: "02", name: "Boarding House Recommendation System", year: "2024", role: "Backend Developer",
    ...
    live: "", github: "https://github.com/newbieflank/web_rekost.git",
    longDesc: "A web-based backend system developed using native PHP to manage users, boarding house owners, and property listings. It provides API endpoints integrated with an Android mobile application, allowing users to browse, search, and view detailed boarding house information seamlessly.",
    highlights: ["Designed database schema for users, owners, and boarding house data", "Developed RESTful APIs for mobile application integration", "Implemented search and filtering logic for listings", "Handled data validation across backend services", "Collaborated with UI/UX team using Figma"],
  ...
  {
    num: "03", name: "Posyandu Management System", year: "2025", role: "Backend Developer",
    ...
    live: "", github: "https://github.com/Mafirrr/Website_Posyandu.git",
    longDesc: "A backend system built with Laravel to manage users, midwives, health cadres, educational content, and Posyandu activity schedules. The system provides RESTful APIs integrated with a Flutter mobile application, enabling efficient access to health check data and schedules.",
    highlights: ["Developed RESTful APIs for mobile app integration", "Designed relational database for healthcare and user data", "Managed authentication and role-based access (admin, midwife, cadre)", "Ensured secure data handling for sensitive information", "Collaborated on system design using Figma"],
  ...
  {
    num: "04", name: "Flood Early Detection System", year: "2025", role: "Backend & System Developer",
    ...
    live: "", github: "https://github.com/newbieflank/FloodSense.git",
    longDesc: "An early warning system designed to detect flood risks using the Mamdani Fuzzy Logic method based on water level and flow velocity parameters. The system integrates an embedded prototype with sensors, while a Flutter mobile application displays real-time conditions and sends notifications.",
    highlights: ["Implemented Mamdani Fuzzy Logic for flood risk classification", "Built real-time monitoring system using embedded sensors", "Developed Flutter app for live status and notifications", "Designed system workflow from hardware to mobile integration", "Simulated real-world environmental monitoring scenario"],
...
function ProjectModal({ project: p, onClose }) {
  ...
  return (
    <div onClick={handleClose} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(10,10,8,0.78)", backdropFilter: "blur(14px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem", animation: `${closing ? "overlayOut" : "overlayIn"} 0.3s ease forwards` }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "var(--surface)", borderRadius: 8, border: "1px solid var(--border2)", width: "100%", maxWidth: 820, maxHeight: "92vh", overflowY: "auto", animation: `${closing ? "modalOut" : "modalIn"} 0.36s cubic-bezier(0.16,1,0.3,1) forwards`, scrollbarWidth: "none" }}>

        {/* Sticky header */}
        <div style={{ padding: "1.75rem 2.5rem", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem", position: "sticky", top: 0, background: "var(--surface)", zIndex: 10, borderRadius: "8px 8px 0 0" }}>
          <div>
            ...
            <h2 style={{ fontFamily: "var(--serif)", fontSize: "clamp(1.4rem,3vw,2rem)", fontWeight: 400, letterSpacing: "-0.01em", lineHeight: 1.15 }}>{p.name}</h2>
          </div>
          <button onClick={handleClose}
            style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: "50%", width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--ink2)", flexShrink: 0, marginTop: 2, transition: "all 0.2s" }}
        ...
        {/* Hero image — dark bg, contain so full screenshot shows */}
        <div style={{ width: "100%", background: "#111", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", minHeight: 220, maxHeight: 400 }}>
          <img src={p.image} alt={p.name} style={{ width: "100%", maxHeight: 400, objectFit: "contain", objectPosition: "top center", display: "block" }} />
        </div>
            ...
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.65rem" }}>
              {p.highlights.map((h, i) => (
                <li key={i} style={{ display: "flex", gap: "1rem", alignItems: "flex-start", fontSize: "0.9rem", color: "var(--ink2)", lineHeight: 1.7, fontWeight: 300 }}>
                  <span style={{ marginTop: "0.65rem", width: 14, height: 1, background: "var(--accent)", flexShrink: 0, display: "inline-block" }} />{h}
                </li>
              ))}
            </ul>
          </div>
...
function Nav({ active }) {
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 5vw", height: 64, background: scrolled ? "rgba(245,244,240,0.93)" : "transparent", backdropFilter: scrolled ? "blur(18px)" : "none", borderBottom: scrolled ? "1px solid var(--border)" : "1px solid transparent", transition: "all 0.35s ease" }}>
      <a href="#hero" style={{ fontFamily: "var(--serif)", fontSize: "1.3rem", fontWeight: 700, color: "var(--ink)", textDecoration: "none" }}>
      ...
      <ul className="nav-links" style={{ display: "flex", gap: "2.5rem", listStyle: "none" }}>
        {["About", "Projects", "Skills", "Contact"].map(l => (
          <li key={l}><a href={`#${l.toLowerCase()}`} className={`nav-link ${active === l.toLowerCase() ? "active" : ""}`}>{l}</a></li>
        ))}
      </ul>
...
function Hero() {
  ...
  return (
    <section id="hero" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "130px 5vw 100px", maxWidth: 1200, margin: "0 auto", position: "relative" }}>
      ...
      <div style={{ ...anim(0.1), display: "flex", marginBottom: "2rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0.4rem 1rem", borderRadius: 99, border: "1px solid rgba(200,169,110,0.4)", background: "rgba(200,169,110,0.08)" }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#4caf50", display: "inline-block", animation: "pulse 2s ease-in-out infinite" }} />
      ...
      <h1 style={{ ...anim(0.2), fontFamily: "var(--serif)", fontSize: "clamp(3.2rem,8vw,7rem)", lineHeight: 1.02, letterSpacing: "-0.03em", fontWeight: 900, marginBottom: "2rem", maxWidth: 900 }}>
        Building <em style={{ fontStyle: "italic", fontWeight: 400, color: "var(--accent)" }}>clean</em>,<br />purposeful<br />
      ...
      <div style={{ position: "absolute", bottom: "3rem", left: "5vw", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, animation: "bob 2.2s ease-in-out infinite" }}>
        <div style={{ width: 1, height: 50, background: "var(--accent)" }} />
        ...
        <div>
          ...
          <FadeIn delay={0.16}>
            ...
            <p style={{ color: "var(--ink2)", fontWeight: 300, lineHeight: 1.88, fontSize: "0.95rem" }}>
              Whether it's designing APIs, managing databases, or developing full-stack applications — I bring consistency, precision, and attention to detail in every project.
            </p>
          </FadeIn>
          <FadeIn delay={0.22}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.5rem", marginTop: "3rem", paddingTop: "2.5rem", borderTop: "1px solid var(--border)" }}>
              {[{ n: "4", l: "Projects completed" }, { n: "3+", l: "Years experience" }, { n: "3+", l: "Happy clients" }].map(s => (
                <div key={s.l} style={{ padding: "1.5rem", background: "var(--surface2)", borderRadius: 4, border: "1px solid var(--border)", transition: "all 0.25s", cursor: "default" }}
...
function ProjectCard({ p, onOpen, index }) {
  return (
    <FadeIn delay={index * 0.09}>
      <div className="project-card" onClick={() => onOpen(p)} role="button" tabIndex={0} onKeyDown={e => e.key === "Enter" && onOpen(p)}>

        {/* Image zone — dark bg so any screenshot color works */}
        ...
        <div className="card-tags">
          {p.tags.slice(0, 3).map(t => (
            <span key={t} style={{ background: "rgba(14,13,11,0.7)", color: "rgba(245,244,240,0.85)", backdropFilter: "blur(6px)", border: "1px solid rgba(200,169,110,0.3)", fontSize: "0.61rem", padding: "0.22rem 0.62rem", borderRadius: 2, fontFamily: "var(--mono)", letterSpacing: "0.07em" }}>{t}</span>
          ))}
        </div>

        {/* Arrow */}
        <div className="card-arrow">
          <svg width="13" height="13" viewBox="0 0 14 14" fill="none"><path d="M3 7h8M7 3l4 4-4 4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </div>

        {/* Info panel — always dark, always white text */}
        <div className="card-panel">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.55rem" }}>
            <p style={{ fontFamily: "var(--mono)", fontSize: "0.59rem", color: "rgba(200,169,110,0.75)", letterSpacing: "0.14em", textTransform: "uppercase" }}>{p.num} / {p.year}</p>
            <p style={{ fontFamily: "var(--mono)", fontSize: "0.59rem", color: "rgba(255,255,255,0.28)", letterSpacing: "0.07em" }}>{p.role}</p>
          </div>
          <h3 style={{ fontFamily: "var(--serif)", fontSize: "1.1rem", fontWeight: 700, color: "#ffffff", letterSpacing: "-0.01em", lineHeight: 1.25, marginBottom: "0.5rem" }}>{p.name}</h3>
          <p style={{ fontSize: "0.79rem", color: "rgba(255,255,255,0.52)", fontWeight: 300, lineHeight: 1.6, marginBottom: "1rem" }}>{p.desc}</p>
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.67rem", color: "var(--accent)", fontFamily: "var(--mono)", letterSpacing: "0.1em", textTransform: "uppercase" }}>
            View details
    ...
    <section id="projects" style={{ padding: "7rem 5vw", maxWidth: 1200, margin: "0 auto", borderTop: "1px solid var(--border)" }}>
      <FadeIn>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "3rem", flexWrap: "wrap", gap: "1rem" }}>
          ...
          <p style={{ color: "var(--ink3)", fontWeight: 300, fontSize: "0.88rem", maxWidth: 260, textAlign: "right", lineHeight: 1.7 }}>Click any project to explore details, stack, and challenges.</p>
        </div>
      </FadeIn>
      ...
      <div className="two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: "5rem", alignItems: "start" }}>
        <div>
          ...
          <FadeIn delay={0.1}>
            ...
            <p style={{ color: "var(--ink2)", fontWeight: 300, lineHeight: 1.88, fontSize: "0.95rem" }}>Always learning, always improving.</p>
          </FadeIn>
          <FadeIn delay={0.18}>
            ...
            <FadeIn key={g.label} delay={i * 0.1}>
              <div style={{ padding: "1.75rem", background: "var(--surface2)", borderRadius: 5, border: "1px solid var(--border)" }}>
                <p style={{ fontFamily: "var(--mono)", fontSize: "0.63rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--accent)", marginBottom: "1rem", display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ width: 16, height: 1, background: "var(--accent)", display: "inline-block" }} />{g.label}
                </p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {g.skills.map(s => <span key={s} className={`skill-pill ${g.featured.includes(s) ? "featured" : ""}`}>{s}</span>)}
                </div>
              </div>
            </FadeIn>
          ))}
    ...
    <section id="contact" style={{ padding: "7rem 5vw", maxWidth: 1200, margin: "0 auto", borderTop: "1px solid var(--border)" }}>
      <div className="two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "start" }}>
        <div>
          ...
          <FadeIn delay={0.1}><p style={{ color: "var(--ink2)", fontWeight: 300, lineHeight: 1.88, marginBottom: "2.5rem", fontSize: "0.95rem" }}>Have a project in mind? I'd love to hear about it. Reach out via any channel or fill the form.</p></FadeIn>
